...
mean, std = {}, {}
mean['ucmd'] = [0.485, 0.456, 0.406]
mean['cifar10'] = [0.485, 0.456, 0.406]
mean['cifar100'] = [x / 255 for x in [129.3, 124.1, 112.4]]

std['ucmd'] = [0.229, 0.224, 0.225]
std['cifar10'] = [0.229, 0.224, 0.225]
std['cifar100'] = [x / 255 for x in [68.2, 65.4, 70.4]]


def get_ucmd(args, alg, name, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    # data_dir = os.path.join(data_dir, name.lower())
    # dset = getattr(torchvision.datasets, name.upper())
    # dset = dset(data_dir, train=True, download=True)
    # data, targets = dset.data, dset.targets

    file = os.path.join(data_dir, name.lower(), 'train')
    folders = os.listdir(file)
    targets = []
    data = np.zeros([len(folders), 32, 32, 3], dtype='uint8')
    print(data.shape)
    i = 0
    for folder in folders:
        targets.append(int(folder[:-7]))
        img = Image.open(file + '/' + folder)
        img = np.array(img)
        data[i] = img
        i += 1
    ...
    transform_strong = transforms.Compose([
        ...
        transforms.RandomCrop(crop_size, padding=int(crop_size * (1 - crop_ratio)), padding_mode='reflect'),
        transforms.RandomHorizontalFlip(),
        RandAugment(args.strong_n, 3),
        transforms.ToTensor(),
        transforms.Normalize(mean[name], std[name])
    ])

    transform_stronger = transforms.Compose([
        transforms.Resize(crop_size),
        transforms.RandomCrop(crop_size, padding=int(crop_size * (1 - crop_ratio)), padding_mode='reflect'),
        transforms.RandomHorizontalFlip(),
        RandAugment(args.stronger_n, 3),
        transforms.ToTensor(),
        transforms.Normalize(mean[name], std[name])
    ])
    ...
    ulb_dset = BasicDataset(alg, ulb_data, ulb_targets, num_classes, transform_weak, True, transform_strong, False, transform_stronger)

    # dset = getattr(torchvision.datasets, name.upper())
    # dset = dset(data_dir, train=False, download=True)
    # test_data, test_targets = dset.data, dset.targets

    file = os.path.join(data_dir, name.lower(), 'test')
    folders = os.listdir(file)
    test_targets = []
    test_data = np.zeros([len(folders), 32, 32, 3], dtype='uint8')
    print(data.shape)
    i = 0
    for folder in folders:
        test_targets.append(int(folder[:-7]))
        img = Image.open(file + '/' + folder)
        img = np.array(img)
        test_data[i] = img
        i += 1
