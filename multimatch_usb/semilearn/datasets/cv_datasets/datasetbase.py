        ...
        if self.transform is None:
            ...
            if not self.is_ulb:
                ...
                if self.alg == 'fullysupervised' or self.alg == 'supervised':
                    ...
                    #     img_w_list.append(self.transform(img))
                    # for i in range(self.k):
                    #     img_s_list.append(self.strong_transform(img))
                    # img_w_list = torch.stack(img_w_list, dim=0)
                    # img_s_list = torch.stack(img_s_list, dim=0)
                    # img_w = self.transform(img)
                    img_s = self.strong_transform(img)
                    transform = transforms.ToTensor()
                    img = transform(img)
                    return {'idx_ulb': idx, 'x_ulb': img, 'x_ulb_w': img_w, 'x_ulb_s': img_s}
                ...
                elif self.alg == 'multimatch':
                    return {'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_s_0': self.strong_transform(img), 'x_ulb_s_1': self.stronger_transform(img)}
                else:
